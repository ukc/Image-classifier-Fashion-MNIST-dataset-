...
class LSTMCell(rnn.RNNCell):
	def __init__(self, n_neurons, input_size, parameters=None):
	...
	def evaluate(self, x, prev_C, prev_h,IG_Wx,IG_Uh,IG_b,FG_Wx,FG_Uh,FG_b,tanh_Wx,tanh_Uh,tanh_b,OG_Wx,OG_Uh,OG_b):
	...
	def __call__(self, x, state, scope=None):
		with tf.variable_scope(scope or type(self).__name__):
			y_t, C_t = self.compute(x,state, self._parameters)	
			return y_t, tf.concat([C_t, y_t], 1)


class GRUCell(rnn.RNNCell):
	def __init__(self, n_neurons, input_size, parameters = None):
		self._n_neurons  = n_neurons
		self._n_inputs   = input_size
		self._parameters = parameters
	...
	def evaluate(self, x, prev_h, UG_Wx, UG_Uh, RG_Wx, RG_Uh, tanh_Wx, tanh_Uh, tanh_b):
	...
	def compute(self, x, state, parameters, scope=None):
		if parameters is None:
			with tf.variable_scope(scope or ("GRU"+str(self._n_neurons))):
				prev_h = state
				...
				tanh_Uh = tf.get_variable("tanh_Uh", [self._n_neurons, self._n_neurons], initializer=xavier_initializer(seed=42))
				tanh_b  = tf.get_variable("tanh_b", [1, self._n_neurons], initializer=tf.ones_initializer())
				return self.evaluate(x, prev_h, UG_Wx, UG_Uh, RG_Wx, RG_Uh, tanh_Wx, tanh_Uh, tanh_b)				
		else:
			prev_h = state
	...
	def __call__(self, x, state, scope=None):
		with tf.variable_scope(scope or type(self).__name__):
			y_t = self.compute(x,state, self._parameters)	
			return y_t, y_t
...
def train_model_gru(n_neurons, n_steps=28, n_inputs = 28,n_outputs=10, learning_rate=0.0001,  n_epochs=100, batch_size=32):
	...
	# lstm-cell
	current_input = tf.unstack(X , n_steps, 1)
	gru_cell_train = GRUCell(n_neurons, n_inputs)
...
def test_model_gru(weight_filepath, n_neurons, n_steps=28, n_inputs = 28,n_outputs=10):
	with tf.Session() as sess:
		...
		UG_Wx   = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/UG_Wx:0')
		UG_Uh   = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/UG_Uh:0')
		RG_Wx   = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/RG_Wx:0')
		RG_Uh   = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/RG_Uh:0')
		tanh_Wx = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/tanh_Wx:0')
		tanh_Uh = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/tanh_Uh:0')
		tanh_b  = sess.run('rnn/GRUCell/GRU'+str(n_neurons)+'/tanh_b:0')
		parameters = {
		"UG_Wx" : UG_Wx,
		"UG_Uh" : UG_Uh,
		"RG_Wx" : RG_Wx,
		...
		# lstm-cell
		current_input = tf.unstack(X , n_steps, 1)
		gru_cell_test = GRUCell(n_neurons, n_inputs, parameters)
...
def test_model_lstm(weight_filepath, n_neurons, n_steps=28, n_inputs = 28,n_outputs=10):
	with tf.Session() as sess:
		...
		FC_W    = sess.run('FC_W:0')
		FC_b    = sess.run('FC_b:0') 
		IG_Wx   = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/IG_Wx:0')
		IG_Uh   = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/IG_Uh:0')
		IG_b    = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/IG_b:0')
		FG_Wx   = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/FG_Wx:0')
		FG_Uh   = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/FG_Uh:0')
		FG_b    = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/FG_b:0')
		OG_Wx   = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/OG_Wx:0')
		OG_Uh   = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/OG_Uh:0')
		OG_b    = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/OG_b:0')
		tanh_Wx = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/tanh_Wx:0')
		tanh_Uh = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/tanh_Uh:0')
		tanh_b  = sess.run('rnn/LSTMCell/LSTM'+str(n_neurons)+'/tanh_b:0') 
		parameters = {
		"IG_Wx" : IG_Wx,
		"IG_Uh" : IG_Uh,
		"IG_b"  : IG_b,
		...
		# lstm-cell
		current_input = tf.unstack(X , n_steps, 1)
		lstm_cell_test = LSTMCell(n_neurons, n_inputs, parameters)
...
if args.model == 'lstm':    
	...
	graphs = [tf.Graph() for x in range(9)]
	values = []
	units = [32, 64, 128, 256]
	c = 0
	for i in units:
		with graphs[c].as_default():
			x = int(i)
			n_neurons = x
			weight_filepath = "./weights/gru/hidden_unit" + str(n_neurons)
			test_acc = test_model_gru(weight_filepath, n_neurons, n_steps=28, n_inputs = 28,n_outputs=10)
			...
			print test_acc
		c += 1
	plt.plot(*zip(*values), label='GRU', marker='o', color='b')
	...
	for i in units:
		with graphs[c].as_default():
			...
			print test_acc	
		c += 1
	plt.plot(*zip(*values), label='LSTM', marker='o', color='r')
